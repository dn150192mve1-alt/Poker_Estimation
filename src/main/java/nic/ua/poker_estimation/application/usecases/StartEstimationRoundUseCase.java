...
@Service
public final class StartEstimationRoundUseCase implements StartEstimationRoundInPort {
    ...
    public StartEstimationRoundUseCase(
    ...
    @Override
    public Room startEstimationRound(UUID roomId, String topic, long durationSeconds) {
        Instant now = Instant.now(clock);
        Room room = loadRoomOutPort.load(roomId)
            .orElseThrow(() -> new NotFoundException("Room not found: " + roomId));
        Room savedRoom = saveRoomOutPort.save(room.startRound(topic, durationSeconds, now));
