...
@Service
public final class SubmitVoteUseCase implements SubmitVoteInPort {
    ...
    @Override
    public Room submitVote(UUID roomId, UUID participantId, String value) {
        Instant now = Instant.now(clock);
        Room room = loadRoomOutPort.load(roomId)
            .orElseThrow(() -> new NotFoundException("Room not found: " + roomId));
        Room savedRoom = saveRoomOutPort.save(room.closeRoundIfExpired(now).submitVote(participantId, value, now));
